...
# Training entrypoint: reads CSVs, trains best model from notebook logic subset, saves artifacts
...
def _detect_label_column(df: pd.DataFrame, explicit: Optional[str] = None) -> str:
    ...
    Priority: explicit (env or arg) -> common aliases -> fuzzy contains 'readmit'.
    ...
    # fuzzy contains 'readmit'
    for c in df.columns:
        if 'readmit' in c.lower():
            return c
    raise ValueError("Could not find a label column. Set LABEL_COL env var or rename your target (e.g., 'readmitted' or 'readmitted_30d').")
...
def prepare_features(
    ...
    # Determine label column (env override allowed)
    ...
    # Eligibility window (30 days after max discharge)
...
def build_pipeline():
    ...
    # Build preprocessors based on the actual training DataFrame provided later
...
def train_and_save(data_dir: str = 'data', artifacts_dir: str = 'artifacts'):
    ...
    claims, patients = load_data(Path(data_dir))
